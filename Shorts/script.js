...
allReels.addEventListener("click", function (e) {
  if (e.target.className == "right") {
    if (!videos[e.target.id].videolike) {
      videos[e.target.id].likeCount++;
      videos[e.target.id].videolike = true;
    } else {
      videos[e.target.id].likeCount--;
      videos[e.target.id].videolike = false;
    }
  }
  console.log(e.target.className)

  if (e.target.className == "follow") {
    if (!videos[e.target.id].isFollow) {
      videos[e.target.id].isFollow = true;
    } else {
      videos[e.target.id].isFollow = false;
    }
  }
  loadVideos();
